...
**단계: 전 기능 구현 + 실서비스 운영 중. 핵심 버그(교사 화면 멈춤) 수정 완료.**

- 배포본: https://gyeonggi-startup.vercel.app (서울 리전, keep-alive 동작)
- 교사 로그인: babo@babosam.net / 987612 (= SUPER_ADMIN_EMAIL, 슈퍼어드민)
- 전 기능 타입체크·빌드 통과, CI 자동 검사 동작.
- 브랜치 → PR → merge 워크플로 확립 (main 직접 push 금지).
- 2026-07-09: `router.refresh()` 제거로 교사 화면 멈춤 수정, reflections 구독 필터 추가.

다음 권장: 실제 수업에서 교사 화면 멈춤 재현 여부 확인.
